...
class NetlifyWebSocketClient {
    ...
    sendGameMessage(type, data) {
        console.log('Sending game message:', { type, data });
        this.sendMessage('game-message', { messageType: type, data });
        
        // For demo purposes, simulate opponent responses
        if (type === 'code-set') {
            // Simulate opponent setting their code after a delay
            setTimeout(() => {
                if (this.onMessage) {
                    console.log('Simulating opponent code set');
                    this.onMessage({
                        type: 'game-message',
                        data: {
                            messageType: 'code-set',
                            playerNumber: this.isHost ? 2 : 1,
                            code: ['red', 'blue', 'green', 'yellow'] // Simulated opponent code
                        }
                    });
                }
