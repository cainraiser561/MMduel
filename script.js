class MastermindDuel {
    ...
    initializeWebSocket() {
        // Try GitHub Pages WebSocket solution using Firebase
        this.updateConnectionStatus('Connecting to GitHub Pages server...');
        
        try {
            this.githubClient = new GitHubWebSocketClient(
                this.roomCode,
                this.playerNumber === 1,
                () => this.onGitHubConnect(),
                (message) => this.onGitHubMessage(message),
                () => this.onGitHubDisconnect(),
                (error) => this.onGitHubError(error)
            );
            
        } catch (error) {
            console.error('GitHub WebSocket failed:', error);
            this.updateConnectionStatus('GitHub server not available, using simulation...');
            this.fallbackToSimulation();
        }
    }
    
    onGitHubConnect() {
        this.updateConnectionStatus('✅ Connected to GitHub Pages server!');
    ...
    onGitHubMessage(message) {
        console.log('GitHub WebSocket message received:', message);
    ...
    onGitHubDisconnect() {
        this.updateConnectionStatus('❌ GitHub connection lost');
    ...
    onGitHubError(error) {
        ...
        this.updateConnectionStatus('❌ GitHub connection failed');
    ...
    sendGameMessage(type, data) {
        if (this.githubClient && this.githubClient.connected) {
            this.githubClient.sendGameMessage(type, data);
        }
    }
