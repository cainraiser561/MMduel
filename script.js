class MastermindDuel {
    ...
    initializeWebSocket() {
        // Try internet WebSocket for cross-network play
        this.updateConnectionStatus('Connecting to internet server...');
        
        try {
            this.internetClient = new InternetWebSocketClient(
                this.roomCode,
                this.playerNumber === 1,
                () => this.onInternetConnect(),
                (message) => this.onInternetMessage(message),
                () => this.onInternetDisconnect(),
                (error) => this.onInternetError(error)
            );
            
        } catch (error) {
            console.error('Internet WebSocket failed:', error);
            this.updateConnectionStatus('Internet server not available, using simulation...');
            this.fallbackToSimulation();
        }
    }
    
    onInternetConnect() {
        this.updateConnectionStatus('✅ Connected to internet server!');
        this.updateConnectionStatus('🌐 Cross-network multiplayer active');
        this.updateConnectionStatus('🔍 Searching for opponent...');
        this.connected = true;
        
        // The actual connection happens when both players are discovered
        // This will be called when the InternetWebSocketClient finds the other player
    }
    
    onInternetMessage(message) {
        console.log('Internet WebSocket message received:', message);
        
        switch (message.type) {
            case 'player-found':
                this.updateConnectionStatus('🎉 Opponent found!');
                this.updateConnectionStatus('🔗 Establishing secure connection...');
    ...
    onInternetDisconnect() {
        this.updateConnectionStatus('❌ Internet connection lost');
    ...
    onInternetError(error) {
        ...
        this.updateConnectionStatus('❌ Internet connection failed');
        this.fallbackToSimulation();
    }
    ...
    sendGameMessage(type, data) {
        if (this.internetClient && this.internetClient.connected) {
            this.internetClient.sendGameMessage(type, data);
        }
    }
